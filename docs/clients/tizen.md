...
> [!TIP]
> The Phlex app for Samsung Smart TVs runs on 2018 and newer Samsung TVs powered by Tizen. Enable Developer Mode on your TV, sideload the `.wgt` package from a USB drive, then open the app and enter your server URL or sign in with your Hub account.

## Install / Store Links

- **Samsung Galaxy Store:** [Phlex on Galaxy Store](#) <!-- placeholder: pending Samsung store submission -->
- **Direct `.wgt` download:** [github.com/detain/phlex-tizen-client/releases](https://github.com/detain/phlex-tizen-client/releases) — for sideloading
...
1. Download the latest `.wgt` file from [github.com/detain/phlex-tizen-client/releases](https://github.com/detain/phlex-tizen-client/releases) to a computer.
...
7. Confirm the installation. The Phlex app icon appears in **My Apps**.

### Step 3 — Network Requirement

The TV must be on the same local network as your Phlex server, or connected via the Hub relay for remote access. Avoid connecting the TV to a "guest" network that isolates devices from each other.

## Configuration

### In-App Server URL

1. Open **Phlex** from **My Apps**.
2. On first launch, enter your server's address:
   - **Local:** `http://192.168.1.100:32400` (replace with your server's LAN IP)
   - **Hub relay:** `https://hub.phlex.example.com` (after signing in with Hub)
3. The URL is saved automatically. Update it in **Settings → Server URL** later.

### Environment Variable (for Packaged Builds)

For automated or kiosk-style deployments, you can embed the server URL at build time by setting `window.PHLEX_SERVER_URL` in the build configuration. See the [Tizen client readme](https://github.com/detain/phlex-tizen-client/blob/master/README.md) for details.

## Hub Connection

1. In the Phlex app, open **Settings** (gear icon).
2. Navigate to **Hub → Sign In**.
3. Enter the Hub URL (e.g., `https://hub.phlex.example.com`) and press **OK** on your remote.
...
**Fix:** Confirm your TV model year is **2018 or newer**. Phlex requires Tizen 4.0 or later. Update your TV's firmware in **Settings → Support → Software Update**. If your TV is older, use the web client (`http://your-server:32400/web`) in the TV's browser instead.
...
- [Live TV](../advanced/live-tv.md) — watch live television through the Phlex app
